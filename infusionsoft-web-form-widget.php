<?php
/*
Plugin Name: Infusionsoft® Web Form Widget
Plugin URI: http://code.google.com/p/php-infusionsoft-sdk/
...
function infusionsoftWebForm(){
  register_widget('Infusionsoft_Web_Form_Widget');
}

class Infusionsoft_Web_Form_Widget extends WP_Widget {
	function Infusionsoft_Web_Form_Widget() {
		/* Widget settings. */
			...
			if(is_array($webforms)){
				?>
					<p>
						...
						<?php echo $this->getWebFormSelect($webforms, $instance); ?><br/>

						<br/>
						<strong>Note:</strong> Web Form Source will update after you save.
					</p>
					<p>
						<label for="<?php echo $this->get_field_id( 'webformsource' ); ?>">Web Form Source:</label><br/>
							<span style="font-size: 8pt; text-decoration: underline;">Your changes will be preserved until you select a different webform in the above drop down.</span><br/>
...
print_r($e);exit;
	...
	function getWebFormHtml($instance){
        Infusionsoft_AppPool::addApp(new Infusionsoft_App(get_option('infusionsoft_sdk_app_name') . '.infusionsoft.com', get_option('infusionsoft_sdk_api_key')));
		try{
			$html = Infusionsoft_WebFormService::getHTML($instance['webformid']);
		} catch(Exception $e){
